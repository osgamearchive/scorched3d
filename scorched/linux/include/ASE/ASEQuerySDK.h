/*
ASE Query & Reporting SDK           Copyright (C) 2002 UDP Soft Ltd
...
Does a DNS lookup, so call only when net already running. !NOTE! Winsock
must be initialized before calling the init function.
...
These can only be called in the corresponding functions described before.
