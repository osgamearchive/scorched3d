...
bool NetServerTCP2::connect(const char *hostName, int portNo)
{
	...
	{
		Logger::log(formatStringBuffer("NetServerTCP2: Failed to resolve host %s:%i",
			hostName, portNo));
		return false;
	}
	...
	if (!clientSock)
	{
		Logger::log(formatStringBuffer("NetServerTCP2: Failed to open client socket : %s",
			SDLNet_GetError()));
		return false;
	}
...
bool NetServerTCP2::start(int portNo)
{
	...
	if (!serverSock_)
	{
		Logger::log(formatStringBuffer("NetServerTCP2: Failed to open server socket %i", portNo));
		return false;
	}
...
bool NetServerTCP2::startProcessing()
{
	...
	{
		Logger::log(formatStringBuffer("NetServerTCP2: Failed to create NetServerTCP2 thread"));
		return false;
	}
...
int NetServerTCP2::sendRecvThreadFunc(void *c)
{
	...
	Logger::log(formatStringBuffer("NetServerTCP2: shutdown"));
	return 0;
}
