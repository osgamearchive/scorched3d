...
GLVertexArray *ASEArrayFact::getArray(std::list<ASEModel> &modelList, float detail)
{
	std::list<ASEModel *> modelList2;
...
GLVertexTexArray *ASEArrayFact::getTexArray(std::list<ASEModel> &modelList, 
											Vector &max, Vector &min, 
											float detail, TexType texT)
{
	std::list<ASEModel *> modelList2;
	std::list<ASEModel>::iterator itor;
	for (itor = modelList.begin();
	...
	return getTexArray(modelList2, max, min, detail, texT);
}
											...
											float detail, TexType texT)
{
		...
		normalItor != normals.end())
	{
		...
		if (intense > 1.0f) intense = 1.0f; if (intense < 0.2f) intense = 0.2f;
		texCoord = getTexCoord(triA, maxMag, max, min, texT);
		...
		if (intense > 1.0f) intense = 1.0f; if (intense < 0.2f) intense = 0.2f;
		texCoord = getTexCoord(triB, maxMag, max, min, texT);
		...
		if (intense > 1.0f) intense = 1.0f; if (intense < 0.2f) intense = 0.2f;
		texCoord = getTexCoord(triC, maxMag, max, min, texT);
...
Vector ASEArrayFact::getTexCoord(Vector &tri, MaxMag mag, Vector &max, Vector &min, TexType texT)
{
	Vector newTri = tri;
	newTri -= min;
	newTri /= (max - min);
...
void ASEArrayFact::drawTexArray(GLVertexTexArray *texArray)
{
