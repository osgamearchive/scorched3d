...
bool GLFrameBufferObject::create(GLTexture &texture, bool withDepth)
{
	if (!GLStateExtension::hasFBO()) {
		dialogExit("GLFrameBufferObject", "NO FBO");
	}
	...
	if(status != GL_FRAMEBUFFER_COMPLETE_EXT) {
		destroy();
		dialogExit("GLFrameBufferObject", "FBO Init");
	}
...
{
	if (bound_) dialogExit("GLFrameBufferObject", "already bound");
...
{
	if (!bound_) dialogExit("GLFrameBufferObject", "not bound");
