	...
	id_(0)
{
	...
	if (!in) dialogExit("GLSLShader",
		formatString("ERROR: Cannot find shader file \"%s\"", filename));
	...
	{
		dialogExit("GLSLShader", 
			formatString("Shader compiling failed %s : %s", filename, logText.c_str()));
	}
}
