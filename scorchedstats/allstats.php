...
?>
<!-- Script used to limit the number of selections the user can make on a multiple select box -->
	...
	function limitSelection(oSelect, limit){
	     ...
	     for(i ; i < oSelect.length; i++){ 
	          ...
	          if(count > limit){ 
	               ...
	               alert("You're only allowed " + limit + " selections.");
	               break;
	          }
	     }
	}
...
function dataformat($fieldname, $value){
	switch ($fieldname) {
		...
		case 'killratio':
		case 'winratio': return "<td align=center>".prependnumber($value, "%", 2)."</td>";
		case 'timeperconnect':
...
elseif ($dir!="desc" or $dir!="asc") $dir="desc";
...
$urlinfo="allstats.php?Prefix=".$prefix."&PlayerID=0&Fields=".$fields."&Limit=".$limit."&FBy=".$filterurl;
?>
...
<tr><td>
	...
	<tr>
		<td align=center valign=middle>
			<font size="-2">(select up to 8)</font><BR>
...
if ($filterby=='none' or $filterby==Null){
	...
	$wherequery = "WHERE ".$filterby.$filtercompare.$filtervalue;
	$filter=", filter: ".$filterby.$filtercompare.$filtervalue;
}
...
<tr><td align=center><font size="+1"><b><? echo "Players ".($playerid+1)." to ".($playerid+$limit)." (sorted by ".$orderby.$filter.")";?></b></font></td></tr>
</table>
...
echo "<td bgcolor=#111111><b><a href=".$urlinfo."&OrderBy=name&Dir=".sortdirection('name', $orderby, $dir).">Player Name</a></b></td>";
for ($i=0; $i<$num_cols; $i++){
	echo "<td bgcolor=#111111 align=center><b><a href=".$urlinfo."&OrderBy=".$fieldarray[$i]."&Dir=".sortdirection($fieldarray[$i], $orderby, $dir).">".$fieldarray[$i]."</a></b></td>";
}
echo "</tr>";
...
for ($i=1; $i<=$pages; $i++)
{
	...
	echo "<td align=center><font size=-2><a href=$urlinfo&OrderBy=$orderby&Dir=$dir>[$startplayer-$endplayer]</a></font></td>";
	$rows++;
	if ($rows > 7)
	{
		$rows = 0;
